...
<div class="flex flex-wrap w-full  flex-col px-4 py-8 bg-white rounded-lg shadow  sm:px-6 md:px-8 lg:px-10">
    <div class="self-start mb-2 text-xl font-light text-gray-800 sm:text-2xl ">
        ثبت دکتر برای آزمایشگاه
    </div>
    <div class="p-6 mt-8">
        <form action="/admin/doctors/<?= $doctor->id ?>/lab" method="post">
            <div class="flex flex-col mb-2">
                <div class=" relative ">
                    <label for="lab_id" class="mb-2 block font-semibold">آزمایشگاه ها:</label>
                    <select id="lab_id"
                            name="lab_id[]" multiple
                            class=" rounded-lg block flex-1
                                   appearance-none border
                                   <?= errors("lab_id") ? " border-red-600" : "border-gray-300" ?>
                        ...
                        <?php foreach ($labs as $lab) : ?>
                            <option value="<?= $lab->id ?>"
                                <?php foreach ($doctor->labs as $labDoc) {
                                    echo $labDoc->id == $lab->id ? "selected" : "";
                                }
                                ?>>
                                <?= $lab->labName ?>
                            </option>
